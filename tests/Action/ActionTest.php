...
class ActionTest extends TestCase
{
    ...
    {
        $fakeNow = new FakeNow();
        $action = new Lock('a13d4b890111676ba8fb36ece7e94f7d', 'test_lock', $fakeNow);
        $this->assertInstanceOf(ActionInterface::class, $action);
    ...
    {
        $fakeNow = new FakeNow();
        $action = new Unlock('a13d4b890111676ba8fb36ece7e94f7d', 'test_unlock', $fakeNow);
        $this->assertInstanceOf(ActionInterface::class, $action);
        $this->assertSame('/cmd', (string) $action);
        $this->assertArrayHasKey('history', $payload = $action->payload());
        $this->assertArrayHasKey('sign', $payload);
    ...
    {
        $fakeNow = new FakeNow();
        $action = new Toggle('a13d4b890111676ba8fb36ece7e94f7d', 'test_toggle', $fakeNow);
        $this->assertInstanceOf(ActionInterface::class, $action);
        $this->assertSame('/cmd', (string) $action);
        $this->assertArrayHasKey('history', $payload = $action->payload());
        $this->assertArrayHasKey('sign', $payload);
